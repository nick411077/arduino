...
void loop() {
  ...
  switch(state){
    ...
    //第1瓶
        while ( ( (   Ultrasound( 12,11) ) > ( 20 ) ) )     //超音波值大於30cM就循黑線走
          {
          tryFollowLine();              //循黑線
          }
          Left();                       //避左邊
...
{
         if (analogRead(0)<350){iTank.writeMotor(-3,7);}                     //黑線在最左邊
         if (analogRead(0)<350 && analogRead(1)<350 ){iTank.writeMotor(-2,7);}
         if (analogRead(1)<350){iTank.writeMotor(0,5);}                              //黑線在左邊
         ...
         if (analogRead(6)<350){iTank.writeMotor(5,0);}                              //黑線在右邊
         if (analogRead(6)<350 && analogRead(7)<350 ){iTank.writeMotor(7,-2);}
         if (analogRead(7)<350){iTank.writeMotor(7,-3);}                              //黑線在最右邊
         delay(20);
...
void Right(void)
{
  ...
  iTank.writeMotor(4, 4); //前進到瓶子左側
  delay(900);
  iTank.writeMotor(-5, 7); //轉回黑線
  delay(1000);
  while (analogRead(3) > 350)
  {
    iTank.writeMotor(1, 7); //前進到黑線停止
  }
  iTank.writeMotor(4, -4);
  delay(200);
}
