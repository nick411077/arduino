...
void loop() {
  ...
  switch(state){
    case STOP:                // 停止中
      ...
      break;
    case GO:                  // 循軌狀態中
      if(line >=40) {         // 遇到停止線   //數值修正
        iTank.stop();         // 先停車
      //  delay(500);   
       // iTank.writeMotor(1,-1);// 慢速前進
       /*while(line != 0x0) {  // 遇到停止線與校正線之間的空白區域為止
       // getFloorIR(false);  // 持續取得循軌感測值
        }*/
        if (analogRead(7)<350){
    ...
    case CRAB:                // 抓取保特瓶
      ...
      delay(300);
      iTank.stop();           // 停車
      iArm.show(180,180,90,13,10); // 停整手臂到抓取位置並張開手爪
      delay(300);
